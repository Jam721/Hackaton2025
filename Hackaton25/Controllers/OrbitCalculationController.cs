    ...
    : ControllerBase
{
    ...
    public async Task<IActionResult> GetConvergation(int cometId, CancellationToken ct)
    {
        try
        {
            ...
            orbitalParameters.UserEmail = userEmail;
    ...
    [HttpGet("last_params/{cometId:int}")]
    public async Task<IActionResult> LastData(int cometId, CancellationToken ct)
    {
        try
        {
            var observations = await context.Observations
            ...
            closeAppeoach.Distance /= 8;
            
            var userEmail = User.FindFirst(ClaimTypes.Email)?.Value;

            closeAppeoach.UserEmail = userEmail;
